...
function TeamMessages() {
  ...
  }, [activeThreadIds, myProjects, clients, myTasks, myRequests, comments, storeUsers]);
  ...
  return (
    <AppShell role="team" title="Messages" subtitle="Conversations across your projects">
                ...
                <div className="flex flex-wrap items-center gap-1.5 mt-1">
                  ...
                  {activeThread.type === "task" && (
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-muted text-muted-foreground border border-border/60 hover:bg-muted/80 hover:text-foreground transition-all cursor-pointer"
                    >
                      <Eye className="h-3 w-3" /> View task
                    </button>
                  )}
                </div>
              </div>
            ...
            <div ref={scrollContainerRef} className="flex-1 min-h-0 overflow-y-auto p-6 space-y-4 scrollbar-thin">
              {msgs.length === 0 ? (
                ...
                msgs.map((m) => {
                  ...
                  if (!u) return null;
                  const internalMsg = m.visibility === "internal";

                  return (
                    <div key={m.id} className="flex gap-3 text-sm group relative">
                      <UserAvatar user={u} size={32} className="shrink-0" />
                      <div className={cn("flex-1 rounded-2xl px-4 py-3 border border-border/40 transition-all hover:border-border/80 relative", internalMsg ? "bg-amber-500/10 border-amber-500/20" : "bg-muted/40")}>
