...
function TeamSettingsPage() {
  ...
  return (
    <AppShell role="team" title="Settings" subtitle="Manage your portal preferences and account details">
...
function ProfileSection() {
  ...
  const [profile, setProfile] = useState({
    ...
    phone: member.phone ?? "",
    language: "en-US",
    timezone: member.timezone ?? detectTimezone(),
  ...
  // Keep the form in sync if the active member changes.
  useEffect(() => {
    setProfile({
      name: member.name,
      email: member.email,
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      updateTeamMember(member.id, { avatar: dataUrl });
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-6">
          ...
          <div className="relative group cursor-pointer shrink-0" onClick={() => fileInputRef.current?.click()}>
            {isImageAvatar ? (
              ...
              <div className="grid h-16 w-16 place-items-center rounded-full font-bold text-2xl text-white" style={{ backgroundColor: member.color }}>
                {initials || "?"}
              </div>
            )}
          ...
          <div className="space-y-1">
            ...
            <div className="flex gap-2 mt-1">
              ...
              <Button variant="ghost" size="sm" onClick={() => { updateTeamMember(member.id, { avatar: initials }); toast.success("Photo removed"); }}>Remove</Button>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="mt-1 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none"
              >
                {!TIMEZONE_OPTIONS.some((t) => t.tz === profile.timezone) && (
      ...
      <Section title="Location" description="Where you're based — used for scheduling and workspace reference.">
        <div className="space-y-4">
          <Field label="Address" value={profile.address} onChange={set("address")} />
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Field label="City" value={profile.city} onChange={set("city")} />
            <Field label="State" value={profile.state} onChange={set("state")} />
            <Field label="Zip code" value={profile.zipCode} onChange={set("zipCode")} />
          </div>
        </div>
        <div className="mt-6 flex justify-end gap-2">
          <Button onClick={handleSave}>Save changes</Button>
